...
resource "vault_jwt_auth_backend_role" "oidc_role_readonly" {
  ...
  token_policies        = ["default", "kv-${each.value.heritage}-${each.value.csp}-${each.value.asset_id}-readonly"]
  user_claim            = "email"
  role_type             = "oidc"
...
resource "vault_jwt_auth_backend_role" "oidc_role_admin" {
  ...
  token_policies        = ["default", "kv-${each.value.heritage}-${each.value.csp}-${each.value.asset_id}-admin"]
  user_claim            = "email"
  role_type             = "oidc"
  allowed_redirect_uris = ["http://localhost:8200/ui/vault/auth/oidc/oidc/callback", "http://localhost:8250/oidc/callback"]
  groups_claim          = "groups"
